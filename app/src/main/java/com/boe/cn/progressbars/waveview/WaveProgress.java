...
public class WaveProgress extends View {
    ...
    //深色水波颜色
    private int mDarkWaveColor;//颜色不会动态设置变化
    //浅色水波颜色
    private int mLightWaveColor;//颜色不会动态设置变化
    ...
    private int DarkWaveColor;//动态设置波浪颜色
    private int LightWaveColor;//动态设置波浪颜色
    ...
    private void init(Context context, AttributeSet attrs) {
        mDefaultSize = MiscUtil.dipToPx(context, WaveConstant.DEFAULT_SIZE);
        mRectF = new RectF();
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        ...
        mDarkWaveAnimTime = typedArray.getInt(R.styleable.WaveProgress_darkWaveAnimTime, WaveConstant.DEFAULT_ANIM_TIME);
        mLightWaveAnimTime = typedArray.getInt(R.styleable.WaveProgress_lightWaveAnimTime, WaveConstant.DEFAULT_ANIM_TIME);
        mMaxValue = typedArray.getFloat(R.styleable.WaveProgress_maxValue, WaveConstant.DEFAULT_MAX_VALUE);
        mValue = typedArray.getFloat(R.styleable.WaveProgress_value, WaveConstant.DEFAULT_VALUE);
        mValueSize = typedArray.getDimension(R.styleable.WaveProgress_valueSize, WaveConstant.DEFAULT_VALUE_SIZE);
        ...
        mHintSize = typedArray.getDimension(R.styleable.WaveProgress_hintSize, WaveConstant.DEFAULT_HINT_SIZE);

        mCircleWidth = typedArray.getDimension(R.styleable.WaveProgress_circleWidth, WaveConstant.DEFAULT_ARC_WIDTH);
        mCircleColor = typedArray.getColor(R.styleable.WaveProgress_circleColor, mLightWaveColor);
        mBgCircleColor = typedArray.getColor(R.styleable.WaveProgress_bgCircleColor, mLightWaveColor);

        mWaveHeight = typedArray.getDimension(R.styleable.WaveProgress_waveHeight, WaveConstant.DEFAULT_WAVE_HEIGHT);
        mWaveNum = typedArray.getInt(R.styleable.WaveProgress_waveNum, 1);

        mDarkWaveColor = typedArray.getColor(R.styleable.WaveProgress_darkWaveColor, DarkWaveColor);
        mLightWaveColor = typedArray.getColor(R.styleable.WaveProgress_lightWaveColor, LightWaveColor);
    ...
    private void initPaint() {
        ...
        // TODO: 2017/7/5 渐变色
//        LinearGradient gradient = new LinearGradient(250, 0, 250, 300, mDarkWaveColor, mLightWaveColor, MIRROR);
//        RadialGradient radialGradient = new RadialGradient(390, 300, 120f, mDarkWaveColor, mLightWaveColor, MIRROR);
//        mWavePaint.setShader(radialGradient);
...
//        canvas.drawText(String.format("%.0f%%", mPercent * 100), mCenterPoint.x, y, mPercentPaint);// TODO: 2017/7/6 绘制球体中间字体
